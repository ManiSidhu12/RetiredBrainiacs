...
class SavedClassifiedFragment : Fragment(){
    ...
    fun getSavedClassified(){
        val pd = ProgressDialog.show(activity, "", "Loading", false)

        service.getSavedClassified(SharedPrefManager.getInstance(activity!!).userId)
                    ...
                    override fun onNext(t: ClassifiedRoot) {
                        ...
                        if(t != null ){
                            if(t.status.equals("true")) {
                                ...
                                if(t.classified != null && t.classified.size > 0){
                                    v.recycler_stores.adapter = SavedClassifiedAdapter(activity!!,t.classified)
                                }
                            }
                            else{
                                Common.showToast(activity!!,t.message)
